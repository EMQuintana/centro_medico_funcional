...
def cancelar_reserva_paciente(request, reserva_id):
    ...
    disponibilidad = reserva.fecha_reserva
    disponibilidad.reservada = False
    disponibilidad.save()
    reserva.delete()
...
def crear_reserva(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if hasattr(request.user, 'paciente'): return redirect('dashboard_paciente')
...
def gestionar_disponibilidades(request):
    ...
    return render(request, 'fichas_medicas/gestionar_disponibilidades.html', {'form': form, 'disponibilidades': disponibilidades})
...
def modificar_disponibilidad(request):
    if request.method == "POST":
        ...
        disp.fecha_disponible = f"{request.POST.get('fecha')} {request.POST.get('hora')}"
        disp.save()
        return redirect('gestionar_disponibilidades')
...
def api_disponibilidades(request):
    mid = request.GET.get('medico_id')
    medico = get_object_or_404(Medico, id=mid)
    disps = Disponibilidad.objects.filter(medico=medico, ocupada=False, fecha_disponible__gte=now())
    data = [{'id': d.id, 'fecha_hora': localtime(d.fecha_disponible).strftime('%d/%m/%Y %H:%M')} for d in disps]
    return JsonResponse(data, safe=False)
...
def api_validar_rut(request):
    ...
    try:
        ...
        return JsonResponse({'nombre': p.nombre})
